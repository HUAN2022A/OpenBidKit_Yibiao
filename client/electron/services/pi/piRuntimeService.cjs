...
function createPiRuntimeService({ app, configStore, aiService, isMonitorActive, onMonitorEvent, requestUserQuestion }) {
  ...
  async function runTask(payload = {}) {
    ...
    const title = payload.title || '铸标智能体任务';
  ...
  async function runSelfCheck() {
    ...
    try {
      ...
      if (Object.keys(sessionSnapshot).length) {
        setStep('resources', snapshotValidation.resourcesValid ? 'success' : 'error', snapshotValidation.resourcesValid ? '仅加载铸标内置工作区指令' : 'Pi 资源加载结果不符合配置');
      } else {
